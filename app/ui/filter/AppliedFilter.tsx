...
export default function AppliedFilter({
  ...
  if (solved !== undefined) {
    return (
      <div className="bg-white h-400 rounded-md flex flex-col p-5 mx-7 my-2 border-2">
        <div className="text-xs border-1 text-gray-600 bg-gray-100 px-2 py-1 mb-1 mt-1 mr-auto rounded-md flex flex-row">
          {solved === ""
            ? "Todas"
            : solved === "y"
            ? "Resolvidas"
            : "Não Resolvidas"}
          <button onClick={() => onRemoveSelection(name, solved)}>
            <svg
