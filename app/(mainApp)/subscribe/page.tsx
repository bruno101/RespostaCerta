...
const PremiumPlanSVG = () => {
  const id = useId().replace(/:/g, ""); // Remove colons from the ID

  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2000 2000">
      <rect fill="#1FE2EE" width="2000" height="1500" />
      <defs>
        <radialGradient id={`a-${id}`} gradientUnits="objectBoundingBox">
        ...
        />
        <g id={`g-${id}`}>
          <use href={`#s-${id}`} transform="scale(0.12) rotate(60)" />
          <use href={`#s-${id}`} transform="scale(0.2) rotate(10)" />
          <use href={`#s-${id}`} transform="scale(0.25) rotate(40)" />
          <use href={`#s-${id}`} transform="scale(0.3) rotate(-20)" />
          <use href={`#s-${id}`} transform="scale(0.4) rotate(-30)" />
          <use href={`#s-${id}`} transform="scale(0.5) rotate(20)" />
          <use href={`#s-${id}`} transform="scale(0.6) rotate(60)" />
          <use href={`#s-${id}`} transform="scale(0.7) rotate(10)" />
          <use href={`#s-${id}`} transform="scale(0.835) rotate(-40)" />
          <use href={`#s-${id}`} transform="scale(0.9) rotate(40)" />
          <use href={`#s-${id}`} transform="scale(1.05) rotate(25)" />
          <use href={`#s-${id}`} transform="scale(1.2) rotate(8)" />
          <use href={`#s-${id}`} transform="scale(1.333) rotate(-60)" />
          <use href={`#s-${id}`} transform="scale(1.45) rotate(-30)" />
          <use href={`#s-${id}`} transform="scale(1.6) rotate(10)" />
        </g>
      </defs>
      <g transform="">
        <g transform="">
          <circle fill={`url(#a-${id})`} r="3000" />
          <g opacity="0.5">
            <circle fill={`url(#a-${id})`} r="2000" />
            <circle fill={`url(#a-${id})`} r="1800" />
            <circle fill={`url(#a-${id})`} r="1700" />
            <circle fill={`url(#a-${id})`} r="1651" />
            <circle fill={`url(#a-${id})`} r="1450" />
            <circle fill={`url(#a-${id})`} r="1250" />
            <circle fill={`url(#a-${id})`} r="1175" />
            <circle fill={`url(#a-${id})`} r="900" />
            <circle fill={`url(#a-${id})`} r="750" />
            <circle fill={`url(#a-${id})`} r="500" />
            <circle fill={`url(#a-${id})`} r="380" />
            <circle fill={`url(#a-${id})`} r="250" />
          </g>
          <g transform="">
            <use href={`#g-${id}`} transform="rotate(10)" />
            <use href={`#g-${id}`} transform="rotate(120)" />
            <use href={`#g-${id}`} transform="rotate(240)" />
          </g>
          <circle fillOpacity="0.1" fill={`url(#a-${id})`} r="3000" />
        </g>
      </g>
    </svg>
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-purple-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-4xl relative">
        ...
        <div className="hidden md:block absolute top-0 right-0 w-1/2 h-full p-6">
          <div className="h-full flex flex-col">
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(135deg, rgba(0,0,0,1) 0%, rgb(60, 60, 102) 50%, rgb(105, 35, 35) 100%)",
              }}
            >
              {/* CTA Text */}
            ...
            <div className="h-[60%] bg-gray-100 rounded-b-lg px-6 pb-6 pt-10 flex flex-col justify-center">
              <ul
                className={`text-sm font-semibold h-full flex flex-col justify-evenly`}
              >
        ...
        <div className="w-full md:w-1/2">
          <div className="flex flex-col md:hidden mb-7">
            {/* Gradient Section (Top 40%) */}
            <div
              className="h-[250px] w-full rounded-t-lg relative"
              style={{
                background:
                  "linear-gradient(135deg, rgba(0,0,0,1) 0%, rgb(60, 60, 102) 50%, rgb(105, 35, 35) 100%)",
              }}
            >
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                />
                <p
                  className={`${montserrat.className} text-gray-500 font-semibold text-white text-xl w-[90%] text-center`}
                >
                  Assine agora e transforme sua preparação!
                </p>
              </div>
            </div>
            <div className="flex absolute left-1/2 transform -translate-x-1/2 top-[230px] -translate-y-1/2 w-[95%] bg-white rounded-lg shadow-md p-4">
              <div className="h-[70px] w-[70px] rounded-lg min-h-[70px] min-w-[70px] overflow-clip">
                {PremiumPlanSVG()}
              </div>
