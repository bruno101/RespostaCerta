...
export default function DashboardPage() {
  ...
  const handleProfileImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Refresh the session to get the updated profile image
      router.refresh()
    } catch (error) {
  ...
  const handleDeleteAccount = async () => {
    try {
      ...
      router.push("/signout?callbackUrl=/")
    } catch (error) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "pending":
        return <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200"><Clock className="h-3 w-3 mr-1" /> Aguardando</Badge>
      case "graded":
        return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200"><CheckCircle className="h-3 w-3 mr-1" /> Avaliada</Badge>
      case "rejected":
        return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200"><AlertCircle className="h-3 w-3 mr-1" /> Rejeitada</Badge>
      default:
    ...
    <div className="container py-8 px-4 mx-auto">
            ...
            <DialogContent>
              ...
              <div className="flex flex-col items-center gap-4 py-4">
                ...
                <div className="grid w-full max-w-sm items-center gap-1.5">
                  <label htmlFor="profile-image" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
      ...
      <Card className="mb-8 border-t-4 border-t-cyan-500">
        ...
        <CardContent>
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="text-sm text-muted-foreground">{stats.progressToNextLevel}% para o próximo nível</div>
          </div>
          <Progress value={stats.progressToNextLevel} className="h-2" />
          
          <div className="mt-4 text-sm text-muted-foreground">
            Total de pontos: <span className="font-medium text-cyan-700">{stats.totalPoints}</span>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <Card className="bg-gradient-to-br from-cyan-50 to-white border-cyan-100">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-cyan-900">{stats.questionsAnswered}</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-cyan-50 to-white border-cyan-100">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-cyan-900">{stats.commentsPosted}</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-cyan-50 to-white border-cyan-100">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-cyan-900">{stats.currentStreak} dias</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="respostas" className="mb-8">
        ...
        <TabsContent value="respostas">
          <h2 className="text-xl font-semibold mb-4 text-gray-900">Suas Respostas</h2>
              ...
              <CardContent className="pt-6">
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-gray-500 max-w-md mb-6">
                    Você ainda não respondeu nenhuma questão. Comece a responder para acompanhar seu progresso.
                  </p>
                  <Button asChild>
                    <Link href="/questoes">Ver Questões Disponíveis</Link>
                  </Button>
                </div>
              </CardContent>
            </Card>
              ...
              {responses.slice(0, 5).map((response) => (
                <Card key={response.id} className="hover:shadow-md transition-shadow">
                  ...
                  <CardFooter className="pt-0">
                    <Button asChild variant="ghost" size="sm" className="ml-auto text-cyan-700 hover:text-cyan-900 hover:bg-cyan-50">
                      <Link href={`/painel/questoes-respondidas/${response.questionId}`}>
                        Ver detalhes
                      </Link>
                    </Button>
                  </CardFooter>
        ...
        <TabsContent value="atividade">
          <h2 className="text-xl font-semibold mb-4 text-gray-900">Atividade Recente</h2>
      ...
      {/* Account Management */}
      <h2 className="text-xl font-semibold mb-4 text-gray-900">Gerenciamento de Conta</h2>
      <Card>
        <CardContent className="pt-6">
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">Exclua permanentemente sua conta e todos os seus dados</p>
              </div>
              <AlertDialog>
                <AlertDialogTrigger asChild>
